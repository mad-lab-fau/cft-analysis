...
def task_test():
    ...
    return {
        "actions": ["pytest --cov=cft_analysis %(paras)s"],
...
def task_type_check():
    ...
    return {"actions": [["mypy", "-p", "cft_analysis"]], "verbosity": 1}
...
def task_register_ipykernel():
    """Add a jupyter kernel with the cft_analysis env to your local install."""

    return {
        "actions": [
            ["python", "-m", "ipykernel", "install", "--user", "--name", "cft_analysis", "--display-name", "cft_analysis"]
        ]
    }
...
def update_version(version):
    ...
    update_version_strings(HERE / "cft_analysis/__init__.py", new_version)


def task_update_version():
    """Bump the version in pyproject.toml and cft_analysis.__init__ ."""
    return {
